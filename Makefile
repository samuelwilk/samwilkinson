...
tailwind:
	...
	docker compose exec app php bin/console tailwind:dev --watch
...
test-db-reset:
	...
	docker compose exec app sh -c "APP_ENV=test php bin/console doctrine:schema:drop --force --full-database 2>/dev/null || true"
...
test-coverage:
	...
	docker compose exec app sh -c "XDEBUG_MODE=coverage php bin/phpunit --coverage-html var/coverage"
